...
class LandmarkGroups(Transform):
    def __init__(
        self, transforms: Transform | list[Transform], lengths: list[int], **kwargs: Any
    ):
        super().__init__(**kwargs)
        self.transforms = transforms
        ...
        if isinstance(transforms, Transform):
            self.transforms = [self.transforms] * len(lengths)
...
class AlignedFrameRoll(Transform):
    ...
    def apply(
        ...
        landmarks = landmarks.roll(int(roll * landmarks.size(0)), dims=0)
